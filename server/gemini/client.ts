...
export function generateInstantResolution(
  ...
  if (customer.id === 'meher') {
    ...
    if (guardrail.intent === 'EXPLAIN_FULL_NIGHT_RESTRICTION') {
      return "Airline service rules specify that for daytime flight delays, hotel accommodation is limited to the delayed hours portion prior to departure. Frontline agents cannot approve a full night stay for a daytime delay. If you would like to request an exception, I can escalate your case to a supervisor.";
    }
...
CUSTOMER-SPECIFIC DISRUPTION CONTEXT:
${
  customer.id === 'priya'
...
- Disruption remedies apply strictly to the cancelled flight. Free Business Class upgrade on unaffected return flight is NOT permitted under policy. Frontline agent cannot authorize upgrades on unaffected flights; offer supervisor escalation if requested.
- Remedies: Choice between 1) Free priority rebooking on next available flight within 24 hours, or 2) Full refund credited to original payment method in 7 business days. Gold tier receives priority rebooking, NO cash compensation.`
    : customer.id === 'arvind'
...
- Remedies: Complimentary meal voucher + departure lounge access at Mumbai airport.
- Hotel accommodation is NOT eligible (airline policy strictly requires delay > 5 hours; a 4-hour delay does not qualify).`
...
- Remedies: Complimentary meal voucher + departure lounge access + hotel accommodation covering the DELAYED HOURS ONLY (until 20:00 departure), NOT a full night stay.
...
- Specific directive for this turn: ${guardrail.systemDirective}`;
  ...
  // 1. Try Grok (xAI) first if available
